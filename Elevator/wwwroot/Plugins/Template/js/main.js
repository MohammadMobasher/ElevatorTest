...
(function ($) {
	...
	$('.countdown').each(function () {
		...
		$this.countdown(finalDate, function (event) {
			$this.html(event.strftime(
				'<div class="countdown-pack countdown-day"><h3 class="countdown-count">%-D</h3><h6>روز</h6></div>:<div class="countdown-pack countdown-hour"><h3 class="countdown-count">%-H</h3><h6>ساعت</h6></div>:<div class="countdown-pack countdown-minutes"><h3 class="countdown-count">%M</h3><h6>دقیقه</h6></div>:<div class="countdown-pack countdown-seconds"><h3 class="countdown-count">%S</h3><h6>ثانیه</h6></div>'));
		});
	});
	...
	function mailChimpResponse(resp) {
		...
		if (resp.result === 'success') {
			$('.mailchimp-success').html('ارسال با موفقیت انجام شد. ما باید آدرس ایمیل شما را تأیید کنیم. برای تکمیل مراحل اشتراک ، لطفاً روی پیوند موجود در ایمیلی که اخیراً برای شما ارسال کرده ایم ، کلیک کنید.').fadeIn(900);
			$('.mailchimp-error').fadeOut(400);
			
		} else if(resp.result === 'error') {
			$('.mailchimp-error').html('لطفا آدرس ایمیل را درست وارد کنید.').fadeIn(900);
		}  
	}
