...
namespace ElevatorAdminNewUI.TagHelpers
{
    [HtmlTargetElement("TableRowBotton", TagStructure = TagStructure.NormalOrSelfClosing)]
    public class TableRowBotton : TagHelper
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public override Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
                ...
                || ListAccess.Any(a => a.IsAdmin == true))
            {
                ...
                Button = $@"
                    <i class='ptrbl-5 font-size-18 cursorPointer fa {this.Icon} btn-icon data-role-table-row-btn table-btn' {(!string.IsNullOrEmpty(this.QueryString) ? "data-role-querystring='" + this.QueryString + "'" : "")}
                    data-role-href='{(!string.IsNullOrEmpty(this.Area) ? "/" + this.Area : "") + "/" + this.Controller + "/" + this.Action + "/" + this.Id}'
                    data-toggle='tooltip'
                    data-placement='bottom'
                    data-role-wichmodal='{this.WichModal}'
                    title='{this.Title}'
